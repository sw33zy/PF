...
--comprPoligonal' (h:t) = dist' h (last t)

-- 2ºb
fechadaOuAberta' :: Poligonal -> Bool
fechadaOuAberta' [] = True
fechadaOuAberta' [(Cartesiano x y)] = False
--fechadaOuAberta' ((Cartesiano x y):(Cartesiano w z):t) = if fechadaOuAberta_aux (Cartesiano x y) ((Cartesiano w z):t) == True then True else fechadaOuAberta' ((Cartesiano w z):t)
fechadaOuAberta' (h:t) = h==(last t)

fechadaOuAberta_aux :: Ponto -> Poligonal -> Bool
fechadaOuAberta_aux (Cartesiano x y) [] = False
fechadaOuAberta_aux (Cartesiano x y) ((Cartesiano z w):t) = if x==z && y==w then True else fechadaOuAberta_aux (Cartesiano x y) t 
...
triangula' ((Cartesiano x y):(Cartesiano a b):(Cartesiano c d):t) = (Triangulo (Cartesiano x y) (Cartesiano a b) (Cartesiano c d)):triangula' ((Cartesiano x y):(Cartesiano a b):t)
...
auxArea [] = 0
auxArea ((Triangulo (Cartesiano x y) (Cartesiano a b) (Cartesiano c d)):t) =  
--}
...
acrescEmail' n e ((x,y):t) = if n==x then (n,y++[Email e]):t else acrescEmail' n e t

-- 3ºb
verEmails' :: Nome -> Agenda -> Maybe [String]
verEmails' n [] = Nothing
verEmails' n ((x,y):t) = if n==x then Just (selectEmail y) else verEmails' n t
