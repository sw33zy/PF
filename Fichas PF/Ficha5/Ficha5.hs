...
triSup' [] = False
triSup' l = someShit (triAux 0 l)

triAux :: (Eq a,Num a) => Int -> Mat a -> [a]
triAux _ [] = []
triAux x (h:t) = (take x h) ++ triAux (x+1) t

someShit :: (Eq a,Num a) => [a] -> Bool
someShit [] = True
someShit (h:t) | h==0 = someShit t
               | otherwise = False

--3ºh
rotateLeft' :: Mat a -> Mat a
rotateLeft' [] = []
rotateLeft' m = transpose' (rotateAux m)

rotateAux :: Mat a -> Mat a
rotateAux [] = []
